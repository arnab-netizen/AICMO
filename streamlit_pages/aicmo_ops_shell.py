"""
AICMO Ops Shell UI - Diagnostics & E2E Sentinels.
...
Design principles:
  ...
  ✓ All expensive operations behind explicit buttons/actions
  ✓ Lazy imports inside functions (only when user clicks)
  ✓ Evidence-based checks (command output, file existence)
  ✓ Clear pass/fail/unknown status with reasons
...
def render_sentinel_check(check: SentinelCheck) -> None:
    ...
    with st.container():
        ...
        with col2:
            st.write(f"**{check.name}** — {check.details}")
...
def page_sentinels() -> None:
    ...
    if st.button("▶️ Run Sentinels", key="run_sentinels"):
        st.write("")
