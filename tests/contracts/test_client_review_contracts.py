"""
Contract tests for client_review module.
...
def test_client_review_contract_version_exists():
    """CONTRACT_VERSION must exist and equal '1.0'."""
    from aicmo.client_review.api import CONTRACT_VERSION
    
    assert CONTRACT_VERSION == "1.0", f"Expected CONTRACT_VERSION='1.0', got '{CONTRACT_VERSION}'"


def test_client_review_ports_are_abstract():
    """All ports must be ABC subclasses with abstractmethods."""
    import inspect
    from aicmo.client_review import api
...
def test_client_review_dtos_are_pydantic():
...
def test_client_review_events_have_required_fields():
    ...
    if len(event_classes) == 0:
        pytest.skip("No events defined for client_review module yet")
...
def test_client_review_api_has_no_internal_imports():
