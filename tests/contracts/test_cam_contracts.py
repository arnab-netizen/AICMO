...
def test_cam_dtos_are_pydantic():
...
def test_cam_events_have_required_fields():
    ...
    event_classes = [
        ...
        if not name.startswith("_") and name != "DomainEvent" and issubclass(obj, DomainEvent)
    ]
    
    # Events are optional - some modules may not emit events yet
    if len(event_classes) == 0:
        pytest.skip("No events defined for cam module yet")
...
def test_cam_api_has_no_internal_imports():
