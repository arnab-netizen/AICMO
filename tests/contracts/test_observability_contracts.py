...
def test_observability_contract_version_exists():
    """CONTRACT_VERSION must exist and equal '1.0'."""
    from aicmo.observability.api import CONTRACT_VERSION
    
    assert CONTRACT_VERSION == "1.0", f"Expected CONTRACT_VERSION='1.0', got '{CONTRACT_VERSION}'"


def test_observability_ports_are_abstract():
    """All ports must be ABC subclasses with abstractmethods."""
    import inspect
    from aicmo.observability import api
...
def test_observability_dtos_are_pydantic():
...
def test_observability_events_have_required_fields():
    ...
    if len(event_classes) == 0:
        pytest.skip("No events defined for observability module yet")
...
def test_observability_api_has_no_internal_imports():
