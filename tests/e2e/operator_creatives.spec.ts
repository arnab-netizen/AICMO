...
test('Operator Creatives E2E flow', async ({ page }) => {
  ...
  page.on('requestfinished', async (req) => {
    try {
      ...
      if (url.includes('/aicmo/generate') && resp) genRequests.push({ url, status: resp.status() });
      else if (resp) {
