...
def _get_latest_spec(conn, site_id: str) -> Optional[dict]:
    row = conn.execute(
        text("""SELECT spec FROM site_spec WHERE site_id=:sid
...
def save_spec(site_id: str, body: SpecIn) -> SpecResp:
    with db().begin() as conn:
        ...
        if body.name:
            conn.execute(text("UPDATE site SET name=:n WHERE id=:sid"), {"n": body.name, "sid": site_id})
        ver = _next_spec_version(conn, site_id)
        conn.execute(
            text("""INSERT INTO site_spec (site_id, version, spec)
...
async def start_build(site_id: str, req: BuildReq) -> BuildResp:
    ...
    with db().begin() as conn:
        ...
        if not spec:
            raise HTTPException(status_code=400, detail="No spec found for site. POST /sitegen/sites/{site_id}/spec first.")
    ...
    with db().begin() as conn:
        conn.execute(
            text("""INSERT INTO site_build (site_id, workflow_id, run_id, status)
