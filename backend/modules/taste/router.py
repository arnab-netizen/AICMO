...
class AssetOut(BaseModel):
    ...
    def from_orm_asset(cls, a: Asset) -> "AssetOut":
        return cls(
            id=a.id,
            taste_score=float(a.taste_score) if a.taste_score is not None else None,
    ...
    try:
        asset = record_taste(
            ...
            TasteRecord(
                ...
                taste_score=None if payload.taste_score is None else round(payload.taste_score, 1),
                emotion_score=(
                    None if payload.emotion_score is None else round(payload.emotion_score, 2)
                ),
                tone=payload.tone,
                brand_alignment=(
                    None if payload.brand_alignment is None else round(payload.brand_alignment, 2)
                ),
